...
def main(model_ckpt):
    ...
    utils.load_model(model, model_ckpt)
    eval_epoch(data_loader=dataloaders["test"], model=model)
    # model_scripted = torch.jit.script(model)  # Export to TorchScript
    # model_scripted.save(f"{os.path.dirname(model_ckpt)}/model_scripted.pt")  # Save
